    ...
    <div class="main" v-if="article">
      ...
      <div class="like-container">
        ...
        <div v-if="isOwner" class="edit-delete">
          <router-link class="btn btn-outline-primary" :to="'/edit/'+slug">
            <i class="fas fa-edit"></i>
          </router-link>&nbsp;
          <button class="btn btn-outline-primary" v-on:click="deleteArticle">
...
export default {
  ...
  computed: {
    ...
    isOwner() {
      if (
        this.$root.user &&
        this.$root.user.username == this.article.author.username
      ) {
        return true;
      }
      return false;
  ...
  methods: {
    ...
    async deleteArticle() {
      if (
        ...
        const response = ArticleService.deleteArticle(this.slug);
    ...
    async likeDislikePost(slug) {
      ...
      let response = await (this.article.favorited
        ? ArticleService.dislikePost(slug)
        : ArticleService.likePost(slug));
