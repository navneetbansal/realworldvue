<template>
  <div class="articleCard article-preview">
    <div class="article-meta">
      ...
      <div v-bind:class="{ fade: loading}">
        ...
        ></Heart>
        <br />
        <router-link :to="'/article/'+article.slug" class="preview-link">Read More...</router-link>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async likeDislikePost(slug) {
      ...
      const response = await (this.article.favorited
